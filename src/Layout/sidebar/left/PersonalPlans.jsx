    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Personal Health Plans</h1>
            <p className="text-gray-600 mt-2">Your customized health and wellness programs</p>
          </div>
      ...
      <div className="mb-6">
        <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg w-fit">
          {['active', 'upcoming', 'completed', 'all'].map((tab) => (
            ...
            >
              {tab} ({tab === 'all' ? plans.length : plans.filter(p => p.status === tab).length})
            </button>
          ))}
        </div>
      </div>
          ...
          <div key={plan.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
            ...
            <div className={`p-4 ${plan.color} bg-opacity-10`}>
              <div className="flex justify-between items-start mb-2">
                <div>
                  <span className={`px-3 py-1 rounded-full text-xs font-medium text-white ${plan.color}`}>
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">{plan.completedTasks}/{plan.totalTasks}</p>
                </div>
                <div>
              ...
              <div className="flex gap-2">
                <button className="flex-1 bg-indigo-600 text-white py-2 rounded-lg font-medium hover:bg-indigo-700 transition-colors">
                  {plan.status === 'completed' ? 'Review Plan' : 'Continue Plan'}
                </button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">Stay on track with your daily activities</p>
          </div>
          ...
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => {
            ...
            return (
              <div key={day} className="bg-gray-50 rounded-lg p-4">
                <h3 className="font-bold text-gray-900 text-center mb-3">{day}</h3>
                <div className="space-y-2">
                  {dayTasks.length > 0 ? (
                    dayTasks.map((task) => (
                      <div key={task.id} className={`p-2 rounded text-sm ${
                        task.completed ? 'bg-green-100 text-green-800' : 'bg-white border'
                      }`}>
                        <div className="flex items-center gap-2">
                          {task.completed ? (
        ...
        <div className="mt-8">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center gap-2 px-3 py-2 bg-yellow-50 rounded-lg">
              ...
              <span className="text-sm font-medium">Consistency Streak - 7 days</span>
            </div>
            <div className="flex items-center gap-2 px-3 py-2 bg-green-50 rounded-lg">
              <Target size={16} className="text-green-600" />
              <span className="text-sm font-medium">Weight Loss Goal - 5kg</span>
            </div>
